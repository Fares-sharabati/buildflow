...
export default function LoginPage({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    const profile = { ...prof, companies: { name: companyName } }
    onLogin({ user: authData.user, profile })
    setLoading(false)
  }

  return (
    <div style={{ minHeight:'100vh', background:'#0f1117', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:F }}>
      {/* Background grid */}
      <div style={{ position:'fixed', inset:0, zIndex:0, backgroundImage:'linear-gradient(#2a304522 1px, transparent 1px), linear-gradient(90deg, #2a304522 1px, transparent 1px)', backgroundSize:'40px 40px' }}/>
      ...
      <div style={{ position:'relative', zIndex:1, width:'100%', maxWidth:420, padding:'0 20px' }}>
        ...
        {/* Card */}
        <div style={{ background:'#181c27', border:'1px solid #2a3045', borderRadius:16, padding:'32px 28px' }}>
          ...
          {error && (
            <div style={{ background:'#ef44441a', border:'1px solid #ef444444', borderRadius:8, padding:'10px 14px', color:'#ef4444', fontSize:13, marginBottom:20 }}>
              {error}
            </div>
          )}

          <div style={{ marginBottom:16 }}>
            <label style={{ color:'#7a849e', fontSize:11, fontWeight:700, display:'block', marginBottom:6, textTransform:'uppercase', letterSpacing:0.8 }}>Email Address</label>
            <input type="email" value={email} onChange={e=>{ setEmail(e.target.value); setError('') }} onKeyDown={e=>e.key==='Enter'&&handleLogin()} placeholder="you@company.com"
              style={{ width:'100%', background:'#0f1117', border:'1px solid #2a3045', borderRadius:8, padding:'11px 14px', color:'#e8eaf0', fontFamily:F, fontSize:14, outline:'none', boxSizing:'border-box' }}/>
          </div>

          <div style={{ marginBottom:28 }}>
            <label style={{ color:'#7a849e', fontSize:11, fontWeight:700, display:'block', marginBottom:6, textTransform:'uppercase', letterSpacing:0.8 }}>Password</label>
